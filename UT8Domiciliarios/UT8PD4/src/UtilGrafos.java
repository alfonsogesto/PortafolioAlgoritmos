...
public class UtilGrafos {
    ...
    public static void imprimirMatrizWarshall(boolean[][] matriz, Map<Comparable,TVertice> vertices,String titulo) {
        int n = matriz.length;
        Comparable[][] matrizComparable = new Comparable[n][n];

        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                matrizComparable[i][j] = matriz[i][j] ? "1" : "0";
            }
        }

        imprimirMatrizMejoradoParaWarshall(matrizComparable,vertices,titulo);
    }

    public static void imprimirMatrizMejoradoParaWarshall(Comparable[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
        if (vertices != null && matriz.length == vertices.keySet().size()) {
            ...
            for (int i = 0; i < matriz.length; i++) {
                for (int j = 0; j < matriz.length; j++) {
                    if (matriz[i][j] == null) {
                        datos[j] = nulo;
                    } else if (matriz[i][j] instanceof Double && ((Double) matriz[i][j]).equals(Double.MAX_VALUE)) {
                        datos[j] = infinito;
                    } else {
                        datos[j] = matriz[i][j];
                    }
